...
export function TanStackQueryExample() {  const [newTodoTitle, setNewTodoTitle] = useState("")
  ...
  const { data: todos, isLoading, error, isStale, isFetching } = useQuery({
    queryKey: ['todos'],
    queryFn: fetchTodos,
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: (data, variables) => {
      ...
      setSuccessMessage(`✅ Updated todo: "${data.title}" (${data.completed ? 'completed' : 'pending'})`)
    ...
    <Card className="w-full">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          {isFetching && <Badge variant="outline" className="text-xs">Syncing...</Badge>}
          {isStale && <Badge variant="secondary" className="text-xs">Stale</Badge>}
          {(createMutation.isPending || updateMutation.isPending || deleteMutation.isPending) && (
            <Badge variant="outline" className="text-xs">Mutating...</Badge>
          )}
      ...
      <CardContent className="space-y-4">        <div className="flex gap-2">
          <Button onClick={refetchTodos} disabled={isFetching} variant="outline">
            ...
            Array.from({ length: 5 }).map((_, i) => (
              ...
              <div 
                ...
                className={`flex items-center space-x-2 p-2 border rounded group transition-colors ${
                  recentlyUpdated[todo.id] ? 'bg-green-50 border-green-200' : ''
                }`}
              ><button
